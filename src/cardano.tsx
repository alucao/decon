...
const getStakeAddress = async (api: any) => {
  ...
  let stakeAddress = Cardano.Address.from_bytes(Buffer.from(stakeAddressHex, "hex")).to_bech32();
  return stakeAddress;
}

const sendMetadata = async (api: any, metadataJsonObj: any, _username: string, userinfo: string) => {
  
  let metadataLabelKey = new NinjaConfig().getCfg().METADATA_LABEL.toString();

  // new user?
  if (_username) {
    let stakeAddress = await getStakeAddress(api);

    metadataJsonObj.data.from = _username;
    metadataJsonObj.data.userinfo = userinfo;
    metadataJsonObj.data.stake_addr = stakeAddress;
  }

  console.log(yaml.load(metadataJsonObj.data))
  console.log(JSON.stringify(metadataJsonObj.data))
  let cose = await signMessage(api, yaml.dump(metadataJsonObj.data));
  ...
  generalTransactionMetadata.insert(Cardano.BigNum.from_str(metadataLabelKey), metadatum);
...
const sendTransaction = async (api: any, auxiliaryData: any) => {
  ...
  let txBuilder = Cardano.TransactionBuilder.new(
    Cardano.TransactionBuilderConfigBuilder.new()
      ...
      .coins_per_utxo_word(Cardano.BigNum.from_str(protocolParams.coinsPerUtxoWord))
  ...
  let shelleyChangeAddress = Cardano.Address.from_bytes(Buffer.from(changeAddress, "hex"));
  ...
  for (let rawUtxo of rawUtxos) {
    let utxo = Cardano.TransactionUnspentOutput.from_bytes(Buffer.from(rawUtxo, "hex"));
  ...
  txBuilder.add_inputs_from(txUnspentOutputs, txUnspentOutputs.len());
  ...
  txVkeyWitnesses = Cardano.TransactionWitnessSet.from_bytes(Buffer.from(txVkeyWitnesses, "hex"));
...
const signMessage = async (api: any, json: any) => {
  ...
  const cose = await api.signData(stakeAddressHex, Buffer.from(json).toString('hex'));
  ...
  let stakeAddress = Cardano.Address.from_bytes(Buffer.from(stakeAddressHex, "hex"));
  if (address.to_bech32() !== stakeAddress.to_bech32()) {
    console.log("Signature address does not match stake address", address, stakeAddress);
    return;
  }
