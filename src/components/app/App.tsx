...
import { fetchConfig, fetchUserByStakeAddr, fetchUser, fetchPosts, fetchSocials, fetchMessages, fetchCategories, fetchSocialCategories, fetchSubcategories, fetchLocations, fetchSublocations}
  from '../../rest-api';
import { getStakeAddress, sendMetadata } from '../../cardano';
...
Sentry.init({
  ...
  tracePropagationTargets: ["localhost", "localhost:8080", /^https:\/\/yourserver\.io\/api/],
  // Session Replay
...
function App() {
  ...
  React.useEffect(() => {
    ...
    ReactGA.send({ hitType: "pageview", page: window.location.pathname + window.location.search, title: "App" });
  }, []);
  ...
  const {data:posts, error:postsError, isError:postsIsError, isLoading:postsIsLoading } = useQuery<any,any>(['posts', itemsQuery], () => fetchPosts(itemsQuery));
  const {data:socials, error:socialsError, isError:socialsIsError, isLoading:socialsIsLoading } = useQuery<any,any>(['socials', socialsQuery], () => fetchSocials(socialsQuery));
  ...
  const {data:categories, error:categoriesError, isError:categoriesIsError, isLoading:categoriesIsLoading } = useQuery<any,any>('categories', fetchCategories);
  const {data:socialcategories, error:socialcategoriesError, isError:socialcategoriesIsError, isLoading:socialcategoriesIsLoading } = useQuery<any,any>('socialcategories', fetchSocialCategories);
  ...
  const createPost = async (_username, userinfo, title, price, description, category, subcategory, location, sublocation) => {
    ...
    createTransaction(api, metadataJsonObj, _username, userinfo);
  }

  const createSocial = async (_username, userinfo, parent, title, content, socialcategory) => {
    console.log(_username);
    console.log(userinfo);
  ...
  const createTransaction = async (api, metadataJsonObj, _username, userinfo) => {
    try {
      let transactionHash = await sendMetadata(api, metadataJsonObj, _username, userinfo);
      ...
      console.error('Error! Check your account balance, refresh the page and try again? Error: ' + err);
      console.log(err);
  ...
  const renderContent = function(display: Display) {
    switch (display) {
      case Display.Market:
        return <ItemList items={posts} wallet={wallet} selectUser={selectUserQuery} selectMessageTarget={selectMessageTarget} />
      case Display.Messages:
        return <MessageList messages={messages} wallet={wallet} selectUser={selectUserQuery} selectMessageTarget={selectMessageTarget} />
      case Display.Settings:
        return <SettingsList closeSettings={() => setDisplay(Display.Social) } refreshQueries={refreshAllQueries} />;
      default:
        return <SocialList socials={socials} wallet={wallet} selectUser={selectUserQuery} selectSocialTarget={selectSocialTarget} />
    }
  }
    ...
    <div>
      ...
      <div className="App">
        <NavigationBar 
          ...
          selectCategory={display == Display.Market ? selectCategory : selectSocialCategory}
        />
        <div className="d-flex app-contents">
          <div className='app-desktop'>
          {
            display == Display.Market ? <CategoryList categories={categories} selectCategory={selectCategory} />
            : <CategoryList categories={socialcategories} selectCategory={selectSocialCategory} />
          }
          </div>
          { 
       ...
       {
          showAlert ?
          <Alert text={alertText} link={alertLink} type={alertType} closeAlert={closeAlert} /> : 
          ''
        }
        <CreatePostModal username={username} createPost={createPost} 
          categories={categories} subcategories={subcategories} locations={locations} sublocations={sublocations}
        />
        <CreateSocialModal username={username} parent={parentSocial} createSocial={createSocial} 
          socialcategories={socialcategories}
        />
        <CreateMessageModal username={username} to={to} createMessage={createMessage} />
        <UserModal user={user} wallet={wallet} selectMessageTarget={selectMessageTarget} />
        <FaqModal />
      </div>
      <Footer />
    </div>
