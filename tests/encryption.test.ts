...
test('encrypts and decrypts message correctly', async () => {
    const keyPairSrc = await generateKeyPairFromPassword(new TextEncoder().encode("herpa"));
    const keyPairDst = await generateKeyPairFromPassword(new TextEncoder().encode("derpa"));
